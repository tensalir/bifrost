# Heimdall pilot: one monthly file
...
- Figma file named `MONTH YYYY - PerformanceAds` with a page named **"Briefing Template to Duplicate"** and text nodes that have plugin data `heimdallId` set (see [template-setup.md](./template-setup.md)).
- Optional: `HEIMDALL_BATCH_FILE_MAP` env set to a JSON map of canonical month key → Figma file key, e.g. `{"2026-03":"YOUR_FIGMA_FILE_KEY"}`. If omitted, the plugin matches by current file (designer opens the correct monthly file before syncing).
...
- Run the **Heimdall Sync** plugin (Development → Import plugin from manifest → select `figma-plugin/manifest.json`).
...
- In a later iteration, Heimdall can write the new Figma page URL back to a Monday column (e.g. "Link for review"). For the pilot, you can manually paste the link from the new page.
