...
async function runSyncQueuedBriefings(): Promise<{ done: number; failed: string[] }> {
  ...
  const rootAny = root as { loadAsync?: () => Promise<void> }
  if (typeof rootAny.loadAsync === 'function') await rootAny.loadAsync()
  ...
  function fillTextNodes(node: BaseNode, briefing: BriefingPayload): void {
    if (node.type === 'TEXT') {
