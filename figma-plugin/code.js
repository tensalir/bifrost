...
<\/script></body></html>`;
  ...
  function getSubLabelPrefixLength(line) {
    var _a, _b, _c, _d;
    const t = line.trimStart();
    if (t.startsWith("Input visual + copy direction:")) return line.length - t.length + "Input visual + copy direction:".length;
    if (/^Script:\s*/i.test(t)) return line.length - t.length + ((_b = (_a = t.match(/^Script:\s*/i)) == null ? void 0 : _a[0].length) != null ? _b : 0);
    if (/^Copy:\s*/i.test(t)) return line.length - t.length + ((_d = (_c = t.match(/^Copy:\s*/i)) == null ? void 0 : _c[0].length) != null ? _d : 0);
    return 0;
  }
  function getPrimaryLabelPrefixLength(line) {
    const t = line.trimStart();
...
Script:`;
  ...
  async function processJobs(jobs) {
    debugLog = [];
    console.log("[Sync] processJobs START:", jobs.length, "jobs:", jobs.map((j2) => j2.experimentPageName).join(", "));
    var root = figma.root;
    ...
    if (templatePage && typeof templatePage.loadAsync === "function") {
      try {
        ...
        console.warn("[Sync] template loadAsync failed:", e2);
      }
    }
      ...
      try {
        ...
        if (targetPage && typeof targetPage.loadAsync === "function") {
          try {
            ...
            console.warn("[Sync] target page loadAsync failed:", e2);
          }
        }
        ...
        if (hasMapping) {
          ...
          console.log("[Map] applyNodeMapping START:", mappingEntries.length, "entries, root:", contentRoot.name || contentRoot.type);
